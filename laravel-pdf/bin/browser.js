...
const getOutput = async (page, request) => {
    ...
    if (
        ...
        parseInt(request.options.width) === 0 &&
        parseInt(request.options.height) === 0
    ) {
        [request.options.width, request.options.height] = await page.evaluate(function () {
            return [document.documentElement.offsetWidth + 'px', (document.documentElement.scrollHeight + 1) + 'px'];
        });
    }
